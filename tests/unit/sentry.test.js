...
describe('Sentry observability module - Enhanced Scrubbing', () => {
  ...
  describe('hasSensitivePattern', () => {
    ...
    it('identifies API key patterns', () => {
      // Using example/test keys that won't trigger secret scanning
      expect(sentry.hasSensitivePattern('test_key_12345678901234567890')).toBe(true);
      expect(sentry.hasSensitivePattern('api_key_abcdefghijklmnopqrstuvwxyz')).toBe(true);
    });
